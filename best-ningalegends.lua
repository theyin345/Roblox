...
-- ====== 新的克隆+出售逻辑（基于 Red Kitty） ======
_G.autoClone = false
_G.autoClone440 = false

local function cloneAndSellRedKitty(loopCount, switchGetter)
    spawn(function()
        while switchGetter() do
            pcall(function()
                ...
                -- 1. 确保有 Red Kitty，如果没有则购买
                local redKitty = basic:FindFirstChild("Red Kitty")
                if not redKitty then
                    local shopItem = game:GetService("ReplicatedStorage").cPetShopFolder["Red Kitty"]
                    if shopItem then
                        game:GetService("ReplicatedStorage").cPetShopRemote:InvokeServer(shopItem)
                        task.wait(0.1)
                        redKitty = basic:FindFirstChild("Red Kitty")
                    end
                end
                ...
                if redKitty then
                    ...
                    -- 3. 出售 Basic 中所有宠物
                    for _, pet in pairs(basic:GetChildren()) do
...
-- ====== 优化环境（删除金币提示UI）函数（修复：跳过 WindUI） ======
local function cleanCoinUI()
    local function scanAndDestroy(guiParent)
        for _, screenGui in ipairs(guiParent:GetChildren()) do
            if screenGui:IsA("ScreenGui") then
                -- 白名单：跳过 WindUI 以及名称包含 WindUI 的界面
                if screenGui.Name and (screenGui.Name:find("WindUI") or screenGui.Name == "WindUI") then
                    goto continue
                end

                local shouldDestroy = false
                local keywords = {"Cash","Coins","Coin","获得","金币","Gold","Money","奖励"}
                for _, kw in ipairs(keywords) do
                    if screenGui.Name and screenGui.Name:find(kw) then
                        shouldDestroy = true
                        break
                    end
                end
                if not shouldDestroy then
                    for _, child in ipairs(screenGui:GetDescendants()) do
                        if child:IsA("TextLabel") or child:IsA("TextButton") then
                            ...
                            for _, kw in ipairs(keywords) do
                                if text:find(kw) or text:find("%+%d+") then
                                    shouldDestroy = true
                                    break
                                end
                            end
                ...
                if shouldDestroy then
                    screenGui:Destroy()
                    print("已删除金币提示UI: " .. screenGui.Name)
                end
            end
            ::continue::
        end
    ...
    if coreGui then scanAndDestroy(coreGui) end
    if playerGui then scanAndDestroy(playerGui) end
end
-- ==================================================
...
SecBring:Divider()
SecBring:Paragraph({ Title = "强行锁定位置", Desc = "强行锁定位置到山丘" })
SecBring:Toggle({
    Title = "强锁位置",
    Value = false,
    Callback = function(state)
...
SecAuto:Toggle({Title = "自动出售（忍术）", Value = false, Callback = function(state) getgenv().autosell = state end})
...
-- ========== 宠物克隆 Tab（购买+克隆+出售一体化） ==========
local SecPet = TabPet:Section({ Title = "自动宠物克隆", Opened = true })
SecPet:Paragraph({ Title = "上榜用", Desc = "自动购买 Red Kitty → 克隆 → 出售全部 Basic 宠物" })
...
SecPet:Toggle({
    ...
    Callback = function(state)
        _G.autoClone = state
        if state then
            cloneAndSellRedKitty(15, function() return _G.autoClone end)
            WindUI:Notify({Title = "宠物克隆", Content = "已开启60宠物克隆（含出售）", Duration = 4})
        else
            WindUI:Notify({Title = "宠物克隆", Content = "已停止60宠物克隆", Duration = 3})
        end
    end
})
...
SecPet:Toggle({
    ...
    Callback = function(state)
        ...
        if state then
            cloneAndSellRedKitty(10, function() return _G.autoClone440 end)
            WindUI:Notify({Title = "宠物克隆", Content = "已开启440宠物克隆（含出售）", Duration = 4})
        else
            WindUI:Notify({Title = "宠物克隆", Content = "已停止440宠物克隆", Duration = 3})
        end
    end
})
...
SecPet:Button({
    ...
    Callback = function()
        ...
        WindUI:Notify({Title = "优化环境", Content = "已启动金币提示UI清理", Duration = 4})
    end
})
-- ========================================================
